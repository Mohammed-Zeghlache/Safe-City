...
const Chatbot = () => {
  ...
  const sendReportToBackend = async (report) => {
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/api/chat/start`, reportData, {
        headers: {
  ...
  const handleLocationInput = (message) => {
    ...
    if (message.trim().length > 2) {
      setUserLocation({ address: message, manual: true });
      setLocationSubmitted(true);
